...
p_load(tidyverse, gtools, lme4, esc, lmerTest) #load packages
...
merged2 <- merge(demo, pitch, by ="ID") #merging data by the unique ID variable 
...
merged2 %>% group_by(Diagnosis.x) %>% summarise(length(unique(ID)))
...
merged$pause_dur[merged$npause == 0] <- NA #set observations with no pauses to NA
...
merged <- merged %>%
    ...
    mutate(scaled_IQR = as.numeric(scale(IQR)), 
                  ...
                  scaled_pause_dur = as.numeric(scale(scaled_pause_dur)))



merged$Study.x <- as.factor(merged$Study.x)
merged$ID <- as.factor(merged$ID)
merged$Trial <- as.factor(merged$Trial)
merged$Diagnosis.x <- as.factor(merged$Diagnosis.x)
merged$Language <- as.factor(merged$Language)
...
sum_IQR
model_IQR <- as_lmerModLmerTest(model_IQR)
anova(model_IQR)
hedges_g(sum_IQR$coefficients[2,1], totaln = 3998)

model_speak_time <- lmer(scaled_speak_time ~ 1 + Diagnosis.x + (1|ID), data = merged, REML = F,  control = lmerControl(
  optimizer = "nloptwrap",
  calc.derivs = F,
  optCtrl = list(
    ftol_abs = 1e-10,
...
sum_speak_time
hedges_g(sum_speak_time$coefficients[2,1], totaln = 3998)

model_speech_rate <- lmer(scaled_speech_rate ~ 1 + Diagnosis.x + (1|ID), data = merged, REML = F,  control = lmerControl(
  optimizer = "nloptwrap",
  calc.derivs = F,
  optCtrl = list(
    ftol_abs = 1e-10,
...
sum_speech_rate
hedges_g(sum_speech_rate$coefficients[2,1], totaln = 3998)

model_pause_dur <- lmer(scaled_pause_dur ~ 1 + Diagnosis.x + (1|ID), data = merged, REML = F,  control = lmerControl(
  optimizer = "nloptwrap",
  calc.derivs = F,
  optCtrl = list(
    ftol_abs = 1e-10,
...
sum_pause_dur
hedges_g(sum_pause_dur$coefficients[2,1], totaln = 3998)

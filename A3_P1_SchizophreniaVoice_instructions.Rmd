...
length(filter(tbl, tbl$Study == "Study1"))
...
My_f <- function(x){
                        data.frame(Study = regmatches(x, gregexpr('Study[0-9]{1}', x))[[1]], #study
                          ...
                          dur = range(read.delim(x)[,1])[2] - range(read.delim(x)[,1])[1]) #duration
                                  }
...
pitch_data = list.files(path = "C:/Users/jacda/Desktop/Cognitive science/R-stuff/Class work 3/Assignment-3/Assignment-3-1/data/" ,pattern = ".txt") %>% ## NB replace with your path to the files
    purrr::map_df(My_f)
...
demo <- read.csv2("DemographicData.csv", encoding = "UTF-8")


arti <- read.csv("Articulation.txt", encoding = "UTF-8", stringsAsFactors = F)

art_sor <- df[order(df[,1], decreasing = F ),]





tbl_art[,11:18] <- art_sor[,1:8]  
